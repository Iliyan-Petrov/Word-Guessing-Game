...
session_start();
static $Mediumword;
$Mediumword = extractWordFromDatabaseMedium();
static $MediumMeaning;
$MediumMeaning = extractMeaningFromDatabaseMedium($Mediumword);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  elseif (isset($_POST['cooking'])) {
    $category = "кулинария";
    $Mediumword = extractCategoryFromDatabaseMedium($category);
    $MediumMeaning = extractMeaningFromDatabaseMedium($Mediumword);
...
function calculatePoints(sec, click) {
  ...
  if (sec <= 300) { 
    points = 10;
    if(sec < 30) {points *= 3}
    else if(sec < 60) {points *= 2.9}
    else if (sec < 90) {points *= 2.7}
    else if (sec < 120) {points *= 2.5}
    else if (sec < 150) {points *= 2.4}
    else if (sec < 180) {points *= 2.2}
    else if (sec < 210) {points *= 2.1}
  } 
  else if (sec <= 600) { 
    points = 5;
    if (sec < 330) {points *= 2}
    else if(sec < 360) {points *= 1.9}
    else if (sec < 390) {points *= 1.7}
    else if (sec < 420) {points *= 1.5}
    else if (sec < 450) {points *= 1.4}
    else if (sec < 480) {points *= 1.2}
    else if (sec < 510) {points *= 1.1}
  } 
...
function submitForm(event) {
  ...
  var targetWord = "<?php echo $Mediumword; ?>";
...
<tr>
<th> Възможните букви могат да са: </th>
</tr>
<tr>
<td>
<?php
...
foreach ($letters as $letter) {
  $probability = calculateLetterProbability($Mediumword, $letter);
  ...
  while ($count < 2) {
    ...
    if (strpos($Mediumword, $randomLetter) === false) 
    { 
...
function showMessage() { $hint = "<?php echo $MediumMeaning; ?>"; alert($hint); delayedButton.disabled = true;}
...
try {
    ...
    $query = "SELECT game_player, game_points,game_time_seconds FROM games_played WHERE game_difficulty = 'medium' ORDER BY game_points DESC LIMIT 3; ";
