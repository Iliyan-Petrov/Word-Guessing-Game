...
session_start();
static $Hardword;
$Hardword = extractWordFromDatabaseHard();
static $HardMeaning;
$HardMeaning = extractMeaningFromDatabaseHard($Hardword);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  elseif (isset($_POST['cooking'])) {
    $category = "кулинария";
    $Hardword = extractCategoryFromDatabaseHard($category);
    $HardMeaning = extractMeaningFromDatabaseHard($Hardword);
  ...
  elseif (isset($_POST['old_words'])) {
    ...
    $Hardword = extractCategoryFromDatabaseHard($category);
    $HardMeaning = extractMeaningFromDatabaseHard($Hardword);
      echo "<style>body { background-image: url('background images/book.png'); background-size: 220px 220px; background-repeat: no-repeat; }</style>";
      echo '<script> nullTimer() </script>'; 
  }
  ...
  elseif (isset($_POST['4_letter_word'])) {
    header("Location: gameWordEasy.php");
  }
...
<p style="font-size: 1.5em;"> Бързо! Лесно! Интересно!  Познай думата!</p>
        ...
        <input type="submit" name="4_letter_word" value="Познай думата с 4 букви" class="submit-button">
...
function calculatePoints(sec, click) {
  ...
  if (sec <= 300) { 
    points = 10;
    if(sec < 30) {points *= 4}
    else if(sec < 60) {points *= 3.9}
    else if (sec < 90) {points *= 3.7}
    else if (sec < 120) {points *= 3.5}
    else if (sec < 150) {points *= 3.4}
    else if (sec < 180) {points *= 3.2}
    else if (sec < 210) {points *= 3.1}
  } 
  else if (sec <= 600) { 
    points = 5;
    if (sec < 330) {points *= 3}
    else if(sec < 360) {points *= 2.9}
    else if (sec < 390) {points *= 2.7}
    else if (sec < 420) {points *= 2.5}
    else if (sec < 450) {points *= 2.4}
    else if (sec < 480) {points *= 2.2}
    else if (sec < 510) {points *= 2.1}
  } 
  else if (sec <= 900) { 
    points = 2;
    if (sec < 630) {points *= 2}
    else if(sec < 660) {points *= 1.9}
    else if (sec < 690) {points *= 1.7}
    else if (sec < 720) {points *= 1.5}
    else if (sec < 750) {points *= 1.4}
    else if (sec < 780) {points *= 1.2}
    else if (sec < 810) {points *= 1.1}
...
function submitForm(event) {
  ...
  var targetWord = "<?php echo $Hardword; ?>";
...
foreach ($letters as $letter) {
  $probability = calculateLetterProbability($Hardword, $letter);
  ...
  while ($count < 2) {
    ...
    if (strpos($Hardword, $randomLetter) === false) 
    { 
...
function showMessage() { $hint = "<?php echo $HardMeaning; ?>"; alert($hint); delayedButton.disabled = true;}
...
try {
    ...
    $query = "SELECT game_player, game_points,game_time_seconds FROM games_played WHERE game_difficulty = 'hard' ORDER BY game_points DESC LIMIT 3; ";
